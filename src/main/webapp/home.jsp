...
Integer totalTenantsAttr = (Integer) request.getAttribute("totalTenants");
...
Integer pendingFeesAttr = (Integer) request.getAttribute("pendingFees");
Integer pendingComplaintsAttr = (Integer) request.getAttribute("pendingComplaints");
Integer totalCollectionAttr = (Integer) request.getAttribute("totalCollection");
Integer visitorsTodayAttr = (Integer) request.getAttribute("visitorsToday");
Integer totalEmployeesAttr = (Integer) request.getAttribute("totalEmployees");
Integer totalSalaryExpenseAttr = (Integer) request.getAttribute("totalSalaryExpense");
Integer paidTenantsAttr = (Integer) request.getAttribute("paidTenants");
Integer totalOccupiedAttr = (Integer) request.getAttribute("totalOccupied");
Integer totalAvailableAttr = (Integer) request.getAttribute("totalAvailable");
Integer totalExpenseAttr = (Integer) request.getAttribute("totalExpense");
Integer netProfitAttr = (Integer) request.getAttribute("netProfit");

int totalTenants = safeInt(totalTenantsAttr);
int occupiedRooms = safeInt(occupiedRoomsAttr);
int totalRooms = safeInt(totalRoomsAttr);
int pendingFees = safeInt(pendingFeesAttr);
int pendingComplaints = safeInt(pendingComplaintsAttr);
int totalCollection = safeInt(totalCollectionAttr);
int visitorsToday = safeInt(visitorsTodayAttr);
int totalEmployees = safeInt(totalEmployeesAttr);
int totalSalaryExpense = safeInt(totalSalaryExpenseAttr);
int paidTenants = safeInt(paidTenantsAttr);
int totalOccupied = safeInt(totalOccupiedAttr);
int totalAvailable = safeInt(totalAvailableAttr);
int totalExpense = safeInt(totalExpenseAttr);
int netProfit = safeInt(netProfitAttr);
...
<body>
		...
		<div class="sidebar-scroll">
			<div class="nav-group">
				...
				<a class="nav-link active" href="<%=ctx%>/dashboard"
					onclick="closeSidebar()"><i class="fa-solid fa-gauge-high"></i>
					Dashboard</a> <a class="nav-link" href="<%=ctx%>/revenue-chart"
					onclick="closeSidebar()"><i class="fa-solid fa-chart-line"></i>
					Revenue Analytics</a> <a class="nav-link"
					href="<%=ctx%>/monthly-rent-summary" onclick="closeSidebar()"><i
					class="fa-solid fa-chart-pie"></i> Monthly Collection</a> <a
					class="nav-link" href="<%=ctx%>/month-wise-rent-analysis"
					onclick="closeSidebar()"><i class="fa-solid fa-calendar-days"></i>
					Rent Analysis</a>
			</div>

			<div class="nav-group">
				<div class="nav-group-title">Management</div>
				<a class="nav-link" href="<%=ctx%>/fetch-tenants"
					onclick="closeSidebar()"><i class="fa-solid fa-users"></i>
					Tenants</a> <a class="nav-link" href="<%=ctx%>/fetch-rooms"
					onclick="closeSidebar()"><i class="fa-solid fa-bed"></i> Rooms</a>
				<a class="nav-link" href="<%=ctx%>/fetch-fees"
					onclick="closeSidebar()"><i class="fa-solid fa-money-bill-wave"></i>
					Fees</a> <a class="nav-link" href="<%=ctx%>/fetch-payment-requests"
					onclick="closeSidebar()"><i class="fa-solid fa-receipt"></i>
					Payment Requests</a> <a class="nav-link"
					href="<%=ctx%>/fetch-complaints" onclick="closeSidebar()"><i
					class="fa-solid fa-triangle-exclamation"></i> Complaints</a> <a
					class="nav-link" href="<%=ctx%>/fetch-visitors"
					onclick="closeSidebar()"><i class="fa-solid fa-user-group"></i>
			...
			<div class="nav-group">
				...
				<a class="nav-link" href="<%=ctx%>/fetch-paid-tenants"
					onclick="closeSidebar()"><i class="fa-solid fa-circle-check"></i>
					Paid Tenants</a> <a class="nav-link" href="<%=ctx%>/fetch-pending-fees"
					onclick="closeSidebar()"><i class="fa-solid fa-clock"></i>
					Pending Fees</a> <a class="nav-link"
					href="<%=ctx%>/fetch-checkout-history" onclick="closeSidebar()"><i
					class="fa-solid fa-right-from-bracket"></i> Checkout History</a> <a
					class="nav-link" href="<%=ctx%>/fetch-activities"
					onclick="closeSidebar()"><i
					class="fa-solid fa-clock-rotate-left"></i> Activities</a> <a
					class="nav-link" href="<%=ctx%>/fetch-pending-reminders"
					onclick="closeSidebar()"><i class="fa-solid fa-bell"></i>
					Pending Reminders</a>
			</div>
		</div>
	...
	<main class="main">
		<div class="topbar">
			...
			<div class="topbar-right">
				<form class="search-form" action="<%=ctx%>/global-search"
					method="get">
					<i class="fa-solid fa-magnifying-glass"></i> <input
						class="search-input" type="text" name="keyword"
						placeholder="Search tenants, rooms, employees..." required>
					<button class="search-btn" type="submit">Search</button>
				</form>

				<div class="top-icons">
					<a class="icon-chip" href="<%=ctx%>/fetch-activities"
						title="Activities"> <i class="fa-regular fa-bell"></i> <span
					...
					<div class="profile-chip" title="Admin profile">
						...
						<div class="profile-text">
							<strong><%=adminUsername%></strong> <span>Administrator</span>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="hero-grid">
				<div class="hero-main">
					...
					<div class="hero-actions">
						<a class="btn btn-primary" href="<%=ctx%>/revenue-chart"> <i
							...
							href="<%=ctx%>/fetch-payment-requests"> <i
				...
				<div class="hero-side">
					...
					<div class="side-card">
						...
						<div class="big-value"><%=fmt(pendingFees)%></div>
						<div class="subline">
					...
					<div class="side-card">
						...
						<div class="big-value">
							...
							<%=fmt(netProfit)%></div>
						<div class="subline">
							<span>After expenses</span> <span
								class="<%=netProfit >= 0 ? "badge-up" : "badge-down"%>">
			...
			<a class="stat-card" href="<%=ctx%>/fetch-tenants">
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(totalTenants)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(totalRooms)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(paidTenants)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(pendingFees)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(pendingComplaints)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(visitorsToday)%></div>
					</div>
				...
				<div class="stat-top">
					<div>
						...
						<div class="stat-value"><%=fmt(totalEmployees)%></div>
					</div>
			...
			<div class="panel">
					...
					<div class="summary-item profit">
						<div class="row">
							<strong><i class="fa-solid fa-chart-simple"></i> Net
								Profit</strong> <span
								class="<%=netProfit >= 0 ? "badge-up" : "badge-down"%>"><%=netProfit >= 0 ? "Positive" : "Negative"%></span>
						</div>
						<div class="value">
							₹
							<%=fmt(netProfit)%></div>
				...
				<div class="chips">
					<a class="chip" href="<%=ctx%>/monthly-rent-summary"><i
						class="fa-solid fa-chart-pie"></i> Monthly Collection</a> <a
						class="chip" href="<%=ctx%>/month-wise-rent-analysis"><i
						class="fa-regular fa-calendar"></i> Rent Analysis</a> <a class="chip"
						href="<%=ctx%>/fetch-pending-reminders"><i
						class="fa-solid fa-bell"></i> Send Reminders</a> <a class="chip"
						href="<%=ctx%>/fetch-reminder-history"><i
						class="fa-solid fa-clock-rotate-left"></i> Reminder History</a>
				</div>
			</div>
		</section>
	...
	<nav class="bottom-nav">
		<div class="bottom-nav-inner">
			<a class="bottom-item" href="<%=ctx%>/dashboard"> <i
				class="fa-solid fa-house"></i> <span>Home</span>
			</a> <a class="bottom-item" href="<%=ctx%>/fetch-tenants"> <i
				class="fa-solid fa-users"></i> <span>Tenants</span>
			...
			<button class="bottom-item more" type="button"
